...
class RosterUpdate(BaseModel) {status?, notes?, target_enumerator_id?, scheduled_date?}
class BulkImportBody(BaseModel) {form_id*, respondents*}
GET /  →  list_roster()
POST /  →  create_roster()
PATCH /{roster_id}  →  update_roster()
DELETE /{roster_id}  →  delete_roster()
POST /bulk-import  →  bulk_import_roster()
POST /upload-csv  →  upload_roster_csv()
GET /with-status  →  roster_with_status()
```
...
### frontend\src\collect\BeneficiaryListScreen.tsx
```
...
### frontend\src\dashboard\RosterTab.tsx
```
component RosterTab
hook useToast
hook useState
...
### website\_originals_backup\index.html
```
title: FieldGovern — Offline-First Field Data Collection & Analysis for India
nav#navbar
canvas#particleCanvas
...
### website\_originals_backup\privacy.html
```
title: Privacy Policy — FieldGovern
```

### website\_originals_backup\terms.html
```
title: Terms of Service — FieldGovern
```
