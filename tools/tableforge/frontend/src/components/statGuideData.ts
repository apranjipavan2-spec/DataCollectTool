...
export const GUIDES: GuideEntry[] = [
  ...
  },
  {
    ...
    steps: [
      'Frequencies. (% of respondents who chose each option — denominator is N respondents, not N responses.)',
      'Co-occurrence. (Pairs of options frequently chosen together; Jaccard similarity matrix with heat-map data.)',
      'By group. (MR × group cross-tab with per-option χ² + FDR correction across options.)',
      'Exclusive. (% of respondents who chose option X *only* — useful for "single-X" segments.)',
    ],
    outputs: [
    ...
    pitfalls: [
      'Treating an MR column as a normal categorical. (% won\'t sum to 100% because each respondent can contribute to multiple categories.)',
      'For dummy mode, define "truthy values". (The default is Yes / 1 / true — change if your data uses different markers.)',
    ],
    example: '"Select all that apply" question → option-level %s + a co-occurrence matrix showing which options cluster together.',
  },
  {
    id: 'observer',
