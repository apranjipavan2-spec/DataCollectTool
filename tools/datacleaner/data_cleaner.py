...
def ai_correct():
    ...
    from pii_redact import redact_rows, redact_unique_counts, is_pii_column

    pii_cols = [c for c in all_req_cols if is_pii_column(c)]
    if pii_cols:
        return jsonify(error=(
            f"Can't run AI correction on {', '.join(pii_cols)} — this looks like a "
            f"personal-data column (name/phone/email/address/location), and DPDP "
            f"compliance means that content never goes to a third-party AI model. "
            f"Remove it from your column/context selection to continue."
        )), 400
    ...
    if is_row_level:
        ...
        sample_data = redact_rows(prioritized[all_req_cols].to_dict(orient="records"))
        ...
        for c in columns:
            ...
            for val, cnt in vc.items():
                ...
                items[key] = int(cnt)
            col_data[c] = redact_unique_counts(c, items)
