...
async def suggest_tabulation(
    ...
    # Table-design suggestion only needs column shape, not real content — ignore
    # any client-supplied sample_rows and synthesize from declared column types.
    from app.services.ai_sanitize import synthesize_rows
    dummy_rows = synthesize_rows(body.column_headers, n=3) if body.column_headers else []
    try:
...
async def smart_build_tabulation(
    ...
    # Column-design suggestion only needs to know each column's type and declared
    # option labels, never real submission content — so no DB query for real
    # values here at all; the AI sees only schema-derived/synthetic shape.
    from app.services.ai_sanitize import synthesize_rows, dummy_unique_values
    unique_values: dict = {c["id"]: dummy_unique_values(c) for c in selected_cols if c.get("id")}
    sample_rows: list = synthesize_rows(selected_cols, n=3) if selected_cols else []
...
async def _run_ai_generation(
    ...
    try:
            ...
            for section in form.json_schema.get("sections", []):
                for field in section.get("fields", []):
                    ...
                    if col_type in ("single_choice", "multiple_choice", "select", "radio", "checkbox"):
                        # Schema-declared option labels only — never real observed answers.
                        # No AI call in this flow needs to see real submission content
                        # (report/tabulation-suggestion only, see ai_sanitize.py).
                        schema_vals = [
                            o.get("label", o) if isinstance(o, dict) else str(o)
                            for o in col.get("options", [])
                        ]
                        col["values_seen"] = (schema_vals or ["Option A", "Option B", "Option C"])[:20]
        ...
        # Fully synthetic rows built from column type/declared options — no real
        # submission content leaves the app for table-design suggestion at all.
        from app.services.ai_sanitize import synthesize_rows
        sample_rows = synthesize_rows(column_headers, n=3)
...
async def generate_program_report(
    ...
    # Donor report style needs real indicator baseline/target/actual figures, not invented
    # prose — reuse the Results Framework's ITT builder rather than re-querying by hand.
    indicators_block = ""
    if body.style == "donor":
        from app.api.routes.results import _build_itt
        itt = _build_itt(program_id, user["tenant_id"], db)
        lines = []
        for ind in itt["indicators"]:
            wave_bits = []
            for w in itt["waves"]:
                cell = ind["values_by_wave"].get(w["id"], {})
                label = w["wave_label"] or f"Wave {w['wave_number']}"
                target = cell.get("target_value")
                actual = cell.get("actual_value")
                wave_bits.append(f"{label}: target={target if target is not None else '—'}, actual={actual if actual is not None else '—'}")
            lines.append(f"- {ind['name']} ({ind['unit']}): " + "; ".join(wave_bits))
        indicators_block = "\n".join(lines)
