...
def check_feature(tenant_id, feature: str, db: Session):
    ...
    or if the org has explicitly opted out of sending data to third-party AI
    providers (Tenant.ai_config.enabled — defaults true for backward
    compatibility, toggle via PATCH /tenants/{id}/ai-config)."""
    ...
    if feature.startswith("ai_"):
        tenant = db.query(Tenant).filter(Tenant.id == tenant_id).first()
        if tenant and (tenant.ai_config or {}).get("enabled") is False:
            raise HTTPException(
                403,
                "AI features are disabled for your organisation. An org admin can "
