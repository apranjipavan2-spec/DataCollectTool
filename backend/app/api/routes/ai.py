...
# Sub-processor disclosure (DPDP Rule 6 / §6.7 of the compliance plan): the
# HQ country a provider primarily processes API traffic in. Best-public-
# knowledge as of the provider's own published trust/privacy docs — verify
# against their current policy before treating as legally authoritative.
PROVIDER_COUNTRIES = {
...
@router.get("/sub-processor")
def get_ai_sub_processor(db: Session = Depends(get_db)):
    """Public, no-auth: which AI provider (if any) this platform is currently
    configured to use, and its processing country. Never returns key material.
    Used by the public Trust Centre / DPDP compliance page so the disclosure
    stays accurate without a manual edit every time an admin changes provider."""
    ...
    return {
        "configured": True,
        "provider": provider,
        "country": PROVIDER_COUNTRIES.get(provider, "Unknown"),
    }
...
async def generate_report(
    ...
    field_schema = [
        {"id": _field_key(f), "label": f.get("label", f.get("id", "")), "type": f.get("type", "text")}
        for section in (form.json_schema or {}).get("sections", [])
        for f in section.get("fields", [])
    ]
    sub_data = [s.data_json for s in subs if s.data_json]
    from app.services.ai_sanitize import summarize_fields
    field_summary = summarize_fields(field_schema, sub_data)
    ...
    async def _run():
        ...
        with SessionLocal() as sess:
            ...
            try:
                report_md = await ai_service.generate_report(cfg, form_title, field_summary, len(sub_data))
...
def _add_cover_page(doc, db: Session, user: dict, title: str, program_name: str) -> None:
    """Prepend tenant name/logo + report/program title + date — never hard-fails the
    export over a missing/unreachable logo, it just falls back to text-only."""
    ...
    tenant = db.query(Tenant).filter(Tenant.id == user["tenant_id"]).first()
    tenant_name = (tenant.name if tenant else None) or "FieldGovern"
    logo_url = (tenant.logo_url if tenant else "") or ""
    ...
    if logo_url:
        try:
            ...
            pic_p.add_run().add_picture(_io.BytesIO(resp.content), width=Inches(1.5))
        except Exception:
            pass  # no logo — cover page still renders with text only

    name_p = doc.add_paragraph()
    name_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    name_run = name_p.add_run(tenant_name)
    name_run.bold = True
    ...
    if program_name:
        ...
        prog_p.add_run(program_name).font.size = Pt(14)

    date_p = doc.add_paragraph()
    date_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    date_p.add_run(f"Generated on {_date.today().isoformat()}").font.size = Pt(11)


@router.post("/writer/export-docx")
def export_docx(body: dict, user: dict = Depends(require_supervisor), db: Session = Depends(get_db)):
    import io, re
    try:
    ...
    if body.get("include_cover"):
        _add_cover_page(doc, db, user, title, program_name)
        doc.add_page_break()
