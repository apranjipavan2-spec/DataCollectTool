...
async def generate_report(cfg: dict, form_title: str, field_summary: list, total_submissions: int) -> str:
    """field_summary = app.services.ai_sanitize.summarize_fields(...) output — a
    population-level aggregate (value counts / numeric min-max-mean / answered
    counts) per field. No raw respondent row is ever sent for report writing;
    the model only needs patterns across the group, not individual answers."""
    prompt = (
        f"You are a research analyst. Summarize the following field survey results.\n"
        f"Form: {form_title}\nTotal submissions: {total_submissions}\n\n"
        f"Per-field aggregate summary:\n{field_summary}\n\n"
...
REPORT_STYLE_PROMPTS = {
    ...
    "donor": (
        "You are writing a formal donor report for an institutional funder (the kind a program officer "
        "reviews against a grant agreement). Sections, in order: Executive Summary, Program Overview, "
        "Results Against Targets, Key Achievements, Challenges & Mitigation, Financial Overview, "
        "Recommendations, Next Steps. For 'Results Against Targets', use ONLY the indicator baseline/"
        "target/actual figures provided below — never invent numbers. If no indicator data is provided, "
        "write that section as '[REVIEW NEEDED — no indicators defined for this program yet]'. For "
        "'Financial Overview', this platform has no financial data source; write exactly "
        "'[DATA NOT AVAILABLE — attach financials separately]' and nothing else in that section. "
        "Formal, evidence-based, donor-facing tone."
    ),
}
...
async def generate_program_report(
    ...
    indicators_section = f"\nIndicator baseline/target/actual data:\n{indicators_block}\n" if indicators_block else ""
    prompt = (
        f"{style_prompt}\n\n"
