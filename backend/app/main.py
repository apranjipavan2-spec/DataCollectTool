...
# APP_URL is a REQUIRED prod setting (used in email links) and always a real
# https:// domain there — its default is the only one that's still localhost,
# so "APP_URL is a non-localhost URL" is a reliable "this is a real deployment"
# signal without inventing a new required env var nobody's set on the server
# yet. Refuses to start rather than silently running with a guessable JWT
# secret or an open CORS policy in production.
_looks_like_prod = not settings.APP_URL.startswith(("http://localhost", "http://127."))
if _looks_like_prod:
    if settings.JWT_SECRET == "change-me-in-production":
        raise RuntimeError(
            "JWT_SECRET is still the placeholder default. Set a real secret "
            "(python -c \"import secrets; print(secrets.token_hex(48))\") in the "
            "production .env before starting — refusing to start with a "
            "guessable value that would let anyone forge auth tokens."
        )
    _cors = settings.cors_origins
    if not _cors or "*" in _cors:
        raise RuntimeError(
            "CORS_ORIGINS is unset or set to a wildcard ('*') in what looks like "
            "a production deployment (APP_URL=" + settings.APP_URL + "). Set it to "
            "your real domain(s), comma-separated — a wildcard allows any website "
            "to make authenticated requests against this API."
        )
