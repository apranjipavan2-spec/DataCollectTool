...
export default function Dashboard() {
  ...
  const [csvResult, setCsvResult] = useState<{ created: number; skipped: number; errors: string[]; generated_passwords: { phone: string; name: string; password: string }[] } | null>(null)
  ...
  const [compareQueue, setCompareQueue] = useState<DuplicateGroup[] | null>(null)
  ...
  // Drop the just-resolved/dismissed group from the queue and land on whatever
  // slides into its slot, so reviewing "one after another" never requires
  // closing the modal and re-picking the next row from the list.
  const advanceCompareQueue = () => {
    const newQueue = (compareQueue ?? []).filter((_, i) => i !== compareIndex)
    if (newQueue.length === 0) {
      setCompareQueue(null)
      setCompareIndex(0)
    } else {
      setCompareQueue(newQueue)
      setCompareIndex(i => Math.min(i, newQueue.length - 1))
    }
  }
  ...
  const handleRestoreAllDuplicates = async () => {
    if (!confirm(`Restore all ${subsTotal} marked-duplicate submission${subsTotal === 1 ? '' : 's'}? They'll go back to normal (counted in stats/exports again).`)) return
    setRestoringAllDuplicates(true)
    try {
      const params = new URLSearchParams({ page_size: String(subsTotal), page: '1', slim: 'true', duplicate_only: 'true' })
      if (filterForm) params.set('form_id', filterForm)
      if (search) params.set('q', search)
      if (dateFrom) params.set('date_from', dateFrom)
      if (dateTo) params.set('date_to', dateTo)
      const r = await api.get(`/submissions/?${params}`)
      const ids = (r.data.items ?? r.data.submissions ?? []).map((s: Submission) => s.id)
      if (ids.length === 0) { toast.error('Nothing to restore'); return }
      await api.post('/submissions/duplicates/unmark', { submission_ids: ids })
      toast.success(`Restored ${ids.length} submission${ids.length > 1 ? 's' : ''}`)
      loadSubmissions(1, true)
      api.get('/submissions/summary').then(r => setSummary(r.data)).catch(() => {})
    } catch (err: any) {
      toast.error(apiErrorMessage(err, 'Failed to restore all'))
    } finally {
      setRestoringAllDuplicates(false)
    }
  }
  ...
  const renderDuplicateGroups = (groups: DuplicateGroup[]) => {
    // Most-recent received time in a group (empty string sorts last).
    const recency = (g: DuplicateGroup) =>
      g.submissions.reduce((m, s) => (s.server_received_at && s.server_received_at > m ? s.server_received_at : m), '')
    const comparators: Record<typeof dupSort, (a: DuplicateGroup, b: DuplicateGroup) => number> = {
      newest: (a, b) => recency(b).localeCompare(recency(a)),
      oldest: (a, b) => recency(a).localeCompare(recency(b)),
      name: (a, b) => a.enumerator_name.localeCompare(b.enumerator_name) || b.day.localeCompare(a.day),
      count: (a, b) => b.count - a.count || recency(b).localeCompare(recency(a)),
    }
      ...
      <div key={i} className="flex items-center justify-between p-3 bg-catalan-hover rounded border border-catalan-warning/30">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={() => { setCompareQueue(byTier[group.tier]); setCompareIndex(byTier[group.tier].indexOf(group)) }}
          >
    ...
    const active = DUP_TIERS.find(t => t.key === dupTier) ?? DUP_TIERS[0]
    const activeGroups = byTier[active.key]
    const pageStart = (dupTierPage - 1) * DUP_PAGE_SIZE
    const pageGroups = activeGroups.slice(pageStart, pageStart + DUP_PAGE_SIZE)

    return (
      <div>
        {/* Tabs — one per tier, with counts */}
        <div className="flex items-center gap-1 border-b border-catalan-border mb-3">
          {DUP_TIERS.map(t => {
            const count = byTier[t.key].length
            const isActive = t.key === dupTier
            return (
              <button
                key={t.key}
                onClick={() => { setDupTier(t.key); setDupTierPage(1) }}
              ...
              >
                {t.label}
                <span className={`ml-1.5 px-1.5 py-0.5 rounded text-xs ${isActive ? 'bg-catalan-primary/10' : 'bg-catalan-hover'}`}>
                  {count}
                </span>
              </button>
            )
        ...
        {activeGroups.length === 0 ? (
          <div className="text-sm text-catalan-textMuted py-4 text-center">None in this tier</div>
        ) : (
          <>
            <div className="space-y-2">{pageGroups.map(renderGroup)}</div>
            <Pagination page={dupTierPage} pageSize={DUP_PAGE_SIZE} total={activeGroups.length} onChange={setDupTierPage} />
          </>
        )}
      </div>
    )
  ...
  const handleCsvImport = async () => {
    ...
    try {
      ...
      const { data } = await api.post<{ created: number; skipped: number; errors: string[]; generated_passwords: { phone: string; name: string; password: string }[] }>('/users/bulk-import', fd, {
  ...
  return (
    <div className="flex h-screen bg-catalan-bg">
      ...
      {compareQueue && compareQueue[compareIndex] && (
        <DuplicateCompareModal
          key={compareQueue[compareIndex].submission_ids.join(',')}
          group={compareQueue[compareIndex]}
          position={{ index: compareIndex, total: compareQueue.length }}
          onPrev={compareIndex > 0 ? () => setCompareIndex(i => i - 1) : undefined}
          onNext={compareIndex < compareQueue.length - 1 ? () => setCompareIndex(i => i + 1) : undefined}
          onClose={() => setCompareQueue(null)}
          onResolved={refreshAfterDuplicateChange}
          onAdvance={advanceCompareQueue}
        />
      )}
                        ...
                        >
                          <div>
                            ...
                            <p className="text-xs text-catalan-textMuted mt-0.5">
                              {new Date(sub.server_received_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                            </p>
                          </div>
                          <StatusBadge status={sub.status} />
                        </div>
            ...
            <div className="space-y-4">
              ...
              <Card data-help-id="filter-bar">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  ...
                  {!isEnumerator && <div className="flex gap-2 flex-wrap ml-auto items-center" data-help-id="export-btn">
                    ...
                    {showMarkedDuplicates && subsTotal > 0 && (
                      <Button
                        variant="secondary"
                        size="sm"
                        disabled={restoringAllDuplicates}
                        onClick={handleRestoreAllDuplicates}
                        title="Restore every submission currently marked as duplicate (matching the filters above) back to normal"
                      >
                        {restoringAllDuplicates ? 'Restoring…' : `Restore all ${subsTotal}`}
                      </Button>
                    )}
                  </div>}
                </div>
              ...
              {showDuplicates && (
                <Card title="Potential Duplicate Submissions">
                  {loadingDuplicates ? (
                    <div className="text-sm text-catalan-textMuted py-4 text-center">
                      Scanning {(summary?.total ?? 0).toLocaleString()} submissions for duplicates…
                    </div>
                  ) : (
                    <>
                      <p className="text-xs text-catalan-textMuted mb-3">
                        ...
                        {duplicates.length > 0 &&
                          ` — ${duplicates.length.toLocaleString()} groups covering ${duplicates.reduce((n, g) => n + g.count, 0).toLocaleString()} submissions.`}
                      </p>
                ...
                <Card>
                  ...
                  {csvResult && (
                    <div className="mt-3 space-y-2">
                      ...
                      {csvResult.generated_passwords.length > 0 && (
                        <div className="bg-catalan-warning/10 border border-catalan-warning/30 rounded-lg p-3 space-y-2">
                          <div className="flex items-center justify-between">
                            <p className="text-xs font-medium text-catalan-text">
                              {csvResult.generated_passwords.length} user{csvResult.generated_passwords.length !== 1 ? 's' : ''} didn't specify a password — random ones were generated. Save these now, they can't be shown again:
                            </p>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => {
                                const text = csvResult.generated_passwords.map(g => `${g.phone}\t${g.name}\t${g.password}`).join('\n')
                          ...
                          <div className="max-h-40 overflow-y-auto space-y-1">
                            {csvResult.generated_passwords.map((g, i) => (
                              <div key={i} className="text-xs text-catalan-text font-mono flex gap-3">
                                <span>{g.phone}</span>
                                <span className="text-catalan-textMuted">{g.name}</span>
                                <span className="font-semibold">{g.password}</span>
                              </div>
                            ))}
                          </div>
                        </div>
