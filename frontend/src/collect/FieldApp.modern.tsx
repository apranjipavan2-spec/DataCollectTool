...
export default function FieldApp() {
  ...
  const [downloadState, setDownloadState] = useState<'idle' | 'downloading' | 'done' | 'error'>('idle')  // backup-file phase for Submit & download
  const [uploadState, setUploadState] = useState<'idle' | 'uploading' | 'uploaded' | 'offline'>('idle')  // server-sync phase shown on the "Saved!" screen
  ...
  // Most recently saved submission's answers for this form, used by FormRenderer
  // to warn on an identical resubmit. Fully local — works offline.
  const getLastSubmission = useCallback(async (): Promise<Record<string, unknown> | null> => {
    if (!activeForm) return null
    const store = await getStorage()
    const subs = await store.listSubmissions(activeForm.meta.id)
    if (subs.length === 0) return null
    const latest = subs.reduce((a, b) => (a.createdAt > b.createdAt ? a : b))
    return latest.data
  }, [activeForm])
  ...
  const handleSubmit = async (draft: SubmissionDraft) => {
    ...
    // offline → say it's saved as a draft and will upload automatically later.
    setUploadState(navigator.onLine ? 'uploading' : 'offline')
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="text-center max-w-sm w-full">
            ...
            {uploadState === 'uploading' && (
              <div className="inline-flex items-center gap-2 text-xs text-catalan-info bg-catalan-info/10 border border-catalan-info/20 rounded-full px-3 py-1.5 mb-3">
                <span className="w-3.5 h-3.5 border-2 border-catalan-info/40 border-t-catalan-info rounded-full animate-spin" />
                <EmojiIcon e="📡" /> Uploading to server…
              </div>
            )}
            {uploadState === 'uploaded' && (
              <div className="inline-flex items-center gap-2 text-xs text-catalan-success bg-catalan-success/10 border border-catalan-success/20 rounded-full px-3 py-1.5 mb-3">
                <EmojiIcon e="✅" /> Uploaded to server
              </div>
            )}
            {uploadState === 'offline' && (
              <div className="text-xs text-catalan-warning bg-catalan-warning/10 border border-catalan-warning/20 rounded-lg px-3 py-2 mb-3 max-w-xs mx-auto">
                <div className="font-medium flex items-center justify-center gap-1.5">
                  <EmojiIcon e="📴" /> No internet — saved as a draft
                </div>
                <div className="mt-1 text-catalan-textMuted">
                  It will upload automatically when you’re back online. <EmojiIcon e="⏳" />
                  {recoveryPubKey && <> Tip: next time tap <span className="font-medium">💾 Submit &amp; download</span> to keep a backup file too.</>}
                </div>
              </div>
            )}
            {uploadState === 'idle' && (
            ...
            {downloadState !== 'idle' && (
              <div className={`inline-flex items-center gap-2 text-xs rounded-full px-3 py-1.5 mb-8 ${
                downloadState === 'downloading' ? 'text-catalan-info bg-catalan-info/10 border border-catalan-info/20'
                : downloadState === 'done'      ? 'text-catalan-success bg-catalan-success/10 border border-catalan-success/20'
                : 'text-catalan-warning bg-catalan-warning/10 border border-catalan-warning/20'
              }`}>
                {downloadState === 'downloading' && <><span className="w-1.5 h-1.5 rounded-full bg-catalan-info animate-pulse" /> Submitted — downloading your backup file…</>}
                {downloadState === 'done'        && <><EmojiIcon e="💾" /> Backup file saved to this device</>}
                {downloadState === 'error'       && <><EmojiIcon e="⚠" /> Backup download failed — your response is still saved</>}
              </div>
            )}
            {downloadState === 'idle' && <div className="mb-8" />}
      ...
      <div className="flex flex-col h-[100dvh]">
        ...
        <div className="flex-1 min-h-0">
          <FormRenderer
            ...
            onSubmit={async (draft) => { setDownloadState('idle'); await handleSubmit(draft); clearProg() }}
            formId={activeForm.meta.id}
            getLastSubmission={getLastSubmission}
            onSubmitAndDownload={recoveryPubKey && activeForm ? async (draft) => {
              // Submit FIRST — gets the response into the outbox and onto the
              // "Saved!" screen before the slower encrypt/download, so the
              // enumerator sees it's recorded and doesn't re-tap. Then save the
              // encrypted backup file (same id as the outbox record so a later
              // recovery dedups cleanly).
              ...
              } catch (e) { console.error('[FieldApp] submit+download backup failed', e); setDownloadState('error') }
            } : undefined}
