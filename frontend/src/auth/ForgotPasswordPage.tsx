...
export default function ForgotPasswordPage() {
  ...
  const [adminContact, setAdminContact] = useState<{ name: string; phone: string } | null>(null)
  ...
  const handleEnumLookup = async () => {
    ...
    setEnumLoading(true)
    try {
      const { data } = await api.get<{ name: string | null; phone: string | null }>(
        `/auth/admin-contact?phone=${encodeURIComponent(normalized)}`
      )
      setAdminContact(data.phone ? { name: data.name ?? 'Admin', phone: data.phone } : null)
    } catch {
      ...
      <div className="w-full max-w-[420px]">
        ...
        {role === 'enumerator' && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
            ...
            {!lookupDone ? (
              ...
              <div className="mb-4">
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4">
                  <p className="text-sm text-amber-800">We couldn't find an admin linked to that number. Please contact your supervisor directly or use QR login.</p>
                </div>
                <button
                  onClick={() => navigate('/auth/qr-login')}
                  className="w-full py-3 rounded-xl font-semibold text-sm text-white transition-all shadow-md mb-3"
                  style={{ background: 'linear-gradient(135deg, #2563eb, #1d4ed8)' }}
                >
                  📷 Login via QR Code
                </button>
            ...
            {/* QR login always available */}
            {!lookupDone && (
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-5">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Or log in without a password</p>
