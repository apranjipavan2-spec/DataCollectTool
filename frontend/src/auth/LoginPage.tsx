...
export default function LoginPage() {
  ...
  const handleVerifyOtp = async () => {
    ...
    setLoading(true); setError('')
    try {
      const { data } = twoFaMethod === 'totp'
        ? await api.post('/auth/2fa/confirm', { temp_token: tempToken, code: otp.trim() })
        : await api.post('/auth/verify-otp', { phone: pendingPhone, otp: otp.trim() })
      storeUser(data)
      subscribeToPush().catch(() => {})
      const dest = data.role === 'master_admin' ? '/admin'
                 : data.role === 'enumerator'   ? '/collect'
                  ...
                  {twoFaMethod === 'totp' ? (
                    <>
                      <p className="text-sm" style={{ color: '#475569' }}>
                        Enter the 6-digit code from your <strong style={{ color: '#1e293b' }}>authenticator app</strong>
                      </p>
                      <p className="text-xs mt-1" style={{ color: '#94a3b8' }}>Codes refresh every 30 seconds</p>
                    </>
                  ) : (
                    <>
                      <p className="text-sm" style={{ color: '#475569' }}>
