...
export default function UserProfile() {
  ...
  const [totpEnabled,  setTotpEnabled]  = useState<boolean | null>(null)  // null = not loaded yet
  ...
  const handleStartTfaSetup = async () => {
    setTfaError(''); setTfaSaving(true)
    try {
      const { data } = await api.post('/auth/2fa/setup')
      setTfaQr(data.qr_data_url)
      setTfaSecret(data.secret)
      setTfaCode('')
      setTfaStep('setup')
    } catch (err: any) {
      setTfaError(err.response?.data?.detail ?? 'Failed to start 2FA setup')
    } finally {
      setTfaSaving(false)
    }
  }

  const handleVerifyTfaSetup = async () => {
    if (tfaCode.trim().length < 6) return
    setTfaError(''); setTfaSaving(true)
    try {
      await api.post('/auth/2fa/verify', { code: tfaCode.trim() })
      toast.success('Two-factor authentication enabled')
      setTotpEnabled(true)
      setTfaStep('idle'); setTfaCode(''); setTfaQr(''); setTfaSecret('')
    } catch (err: any) {
      setTfaError(err.response?.data?.detail ?? 'Invalid code — check your authenticator app and try again')
    } finally {
      setTfaSaving(false)
    }
  }
  ...
  const handleDisableTfa = async () => {
    ...
    setTfaError(''); setTfaSaving(true)
    try {
      await api.post('/auth/2fa/disable', { code: tfaCode.trim() })
      ...
      <div className="flex-1 flex flex-col overflow-auto">
        ...
        <div className="flex-1 p-6 max-w-2xl mx-auto w-full space-y-6">
          ...
          <Card title="Security">
            ...
            <div className="mt-6 pt-6 border-t border-catalan-border">
              {tfaError && (
                <Alert type="error" message={tfaError} onClose={() => setTfaError('')} className="mb-4" />
              )}

              {tfaStep === 'idle' && (
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-catalan-text font-medium">Two-Factor Authentication</p>
                    <p className="text-xs text-catalan-textMuted mt-0.5">
                      {totpEnabled === null ? 'Loading…' : totpEnabled ? 'Enabled — an authenticator app code is required at login' : 'Not enabled'}
                    </p>
                  </div>
                  {totpEnabled === null ? null : totpEnabled ? (
                    <Button variant="danger" size="sm" onClick={() => { setTfaStep('disable'); setTfaCode(''); setTfaError('') }}>
                      Disable 2FA
                    </Button>
                  ) : (
                    <Button variant="secondary" size="sm" onClick={handleStartTfaSetup} disabled={tfaSaving}>
                      {tfaSaving ? 'Starting…' : 'Enable 2FA'}
                    </Button>
                  )}
                </div>
              )}
              ...
              {tfaStep === 'setup' && (
                <div className="space-y-4">
                  <p className="text-sm text-catalan-text font-medium">Scan with an authenticator app</p>
                  <p className="text-xs text-catalan-textMuted">Google Authenticator, Authy, 1Password, or any TOTP app.</p>
                  {tfaQr && <img src={tfaQr} alt="2FA QR code" className="w-40 h-40 rounded-lg border border-catalan-border" />}
                  <div>
                    <p className="text-xs text-catalan-textMuted mb-1">Can't scan? Enter this code manually:</p>
                    <code className="text-xs bg-catalan-hover px-2 py-1 rounded break-all">{tfaSecret}</code>
                  </div>
                  <div>
                    <label className="text-xs text-catalan-textMuted block mb-1">Enter the 6-digit code from your app</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      maxLength={6}
                      ...
                      onKeyDown={e => { if (e.key === 'Enter') handleVerifyTfaSetup() }}
                      className="w-full bg-catalan-hover border border-catalan-border rounded-lg px-3 py-2 text-sm text-catalan-text outline-none focus:border-catalan-primary tracking-[0.3em] text-center"
                      placeholder="000000"
                    />
                  </div>
                  <div className="flex gap-3">
                    <Button variant="primary" size="sm" onClick={handleVerifyTfaSetup} disabled={tfaSaving || tfaCode.length < 6}>
                      {tfaSaving ? 'Verifying…' : 'Verify & Enable'}
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => { setTfaStep('idle'); setTfaCode(''); setTfaQr(''); setTfaSecret(''); setTfaError('') }}>
                      Cancel
                    </Button>
                  </div>
                </div>
              ...
              {tfaStep === 'disable' && (
                <div className="space-y-4">
                  <p className="text-sm text-catalan-text font-medium">Disable two-factor authentication</p>
                  <p className="text-xs text-catalan-textMuted">Enter a current code from your authenticator app to confirm.</p>
                  <div>
                    <input
                      type="text"
                      inputMode="numeric"
                      ...
                      value={tfaCode}
                      onChange={e => setTfaCode(e.target.value.replace(/\D/g, ''))}
                      onKeyDown={e => { if (e.key === 'Enter') handleDisableTfa() }}
                      className="w-full bg-catalan-hover border border-catalan-border rounded-lg px-3 py-2 text-sm text-catalan-text outline-none focus:border-catalan-primary tracking-[0.3em] text-center"
                      placeholder="000000"
                      autoFocus
                    />
                  ...
                  <div className="flex gap-3">
                    <Button variant="danger" size="sm" onClick={handleDisableTfa} disabled={tfaSaving || tfaCode.length < 6}>
                      {tfaSaving ? 'Disabling…' : 'Confirm Disable'}
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => { setTfaStep('idle'); setTfaCode(''); setTfaError('') }}>
                      Cancel
                    </Button>
                  </div>
                </div>
