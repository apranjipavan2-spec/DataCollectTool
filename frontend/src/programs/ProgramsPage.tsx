...
interface LogframeLevel {
  id: string; parent_id: string | null; level_type: 'goal' | 'outcome' | 'output' | 'activity'
  title: string; description: string; sort_order: number
}
interface Indicator {
  id: string; logframe_level_id: string; code: string; name: string
  unit: string; direction: 'increase' | 'decrease'
  disaggregate_by: string[]; source_form_field: string | null
  aggregation: string; value_source: 'auto' | 'manual'
}
interface IttCell { value_id: string | null; target_value: number | null; actual_value: number | null; value_source: string | null }
interface IttIndicator extends Indicator { values_by_wave: Record<string, IttCell> }
interface IttWave { id: string; wave_number: number | null; wave_label: string | null }
interface IttData { waves: IttWave[]; indicators: IttIndicator[]; logframe: { id: string; title: string; level_type: string; rollup_score: number | null; children: unknown[] }[] }
...
const LEVEL_TYPES = ['goal', 'outcome', 'output', 'activity'] as const

function ResultsPanel({ prog }: { prog: ProgramDetail }) {
  const toast = useToast()
  const [levels, setLevels] = useState<LogframeLevel[]>([])
  const [itt, setItt] = useState<IttData | null>(null)
  const [loading, setLoading] = useState(true)
  const [addingLevel, setAddingLevel] = useState(false)
  const [newLevel, setNewLevel] = useState({ level_type: 'outcome' as LogframeLevel['level_type'], title: '', parent_id: '' })
  const [addingInd, setAddingInd] = useState(false)
  const [newInd, setNewInd] = useState({
    logframe_level_id: '', code: '', name: '', unit: 'percent', direction: 'increase' as 'increase' | 'decrease',
    aggregation: 'percent', source_form_field: '', numerator_field: '', numerator_equals: '',
    disaggregate_by: '', value_source: 'auto' as 'auto' | 'manual',
  })
  ...
  useEffect(() => {
    const formIds = [...new Set(prog.questionnaires.map(q => q.form_id).filter(Boolean))] as string[]
    if (formIds.length === 0) return
    Promise.all(formIds.map(id => api.get(`/forms/${id}`).then(r => r.data).catch(() => null))).then(forms => {
      const keys = new Set<string>()
      for (const f of forms) {
        if (!f?.json_schema) continue
        const allFields = [...(f.json_schema.fields ?? []), ...(f.json_schema.sections ?? []).flatMap((s: { fields?: unknown[] }) => s.fields ?? [])]
        for (const field of allFields as { name?: string; id?: string }[]) {
          const key = field.name || field.id
          if (key) keys.add(key)
        }
      }
      setFormFields([...keys].sort())
    })
  }, [prog.questionnaires])
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [lv, it] = await Promise.all([
        api.get(`/results/programs/${prog.id}/logframe`).then(r => r.data),
        api.get(`/results/programs/${prog.id}/itt`).then(r => r.data),
      ])
      setLevels(lv); setItt(it)
  ...
  const addLevel = async () => {
    ...
    await api.post(`/results/programs/${prog.id}/logframe`, {
      level_type: newLevel.level_type, title: newLevel.title,
      parent_id: newLevel.parent_id || null, description: '', sort_order: levels.length,
    })
    setAddingLevel(false); setNewLevel({ level_type: 'outcome', title: '', parent_id: '' }); load()
  }
  const delLevel = async (id: string) => {
    if (!confirm('Delete this logframe level? Its indicators will be orphaned.')) return
    await api.delete(`/results/programs/${prog.id}/logframe/${id}`); load()
  }

  const addIndicator = async () => {
    if (!newInd.name.trim() || !newInd.logframe_level_id) return
    await api.post(`/results/programs/${prog.id}/indicators`, {
      logframe_level_id: newInd.logframe_level_id, code: newInd.code, name: newInd.name,
      unit: newInd.unit, direction: newInd.direction, aggregation: newInd.aggregation,
      source_form_field: newInd.source_form_field || null,
      numerator_filter: newInd.numerator_field ? { field: newInd.numerator_field, equals: newInd.numerator_equals } : null,
      denominator_filter: null,
      disaggregate_by: newInd.disaggregate_by.split(',').map(s => s.trim()).filter(Boolean),
      value_source: newInd.value_source,
    })
    setAddingInd(false)
    setNewInd({ logframe_level_id: '', code: '', name: '', unit: 'percent', direction: 'increase',
      aggregation: 'percent', source_form_field: '', numerator_field: '', numerator_equals: '', disaggregate_by: '', value_source: 'auto' })
    load()
  }
  const delIndicator = async (id: string) => {
    if (!confirm('Delete this indicator?')) return
    await api.delete(`/results/programs/${prog.id}/indicators/${id}`); load()
  }
  const recompute = async (id: string) => {
    setComputing(id)
    try {
      await api.post(`/results/programs/${prog.id}/indicators/${id}/compute`)
      toast.success('Recomputed'); await load()
    } finally { setComputing(null) }
  }
  const saveManual = async (valueId: string, field: 'target_value' | 'actual_value', raw: string) => {
    const v = raw === '' ? null : Number(raw)
    await api.put(`/results/programs/${prog.id}/indicator-values/${valueId}`, { [field]: v })
    load()
  }
  const exportXlsx = async () => {
    setExporting(true)
    try {
      const r = await api.get(`/results/programs/${prog.id}/itt/xlsx`, { responseType: 'blob' })
      const url = URL.createObjectURL(r.data)
      const a = document.createElement('a'); a.href = url; a.download = 'indicator_tracking_table.xlsx'; a.click()
      URL.revokeObjectURL(url)
    } finally { setExporting(false) }
  }

  const levelLabel = (id: string) => levels.find(l => l.id === id)?.title ?? '—'

  if (loading) return <p className="text-center text-catalan-textMuted py-8">Loading…</p>
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-semibold text-xs text-catalan-textMuted uppercase tracking-wide">Results Framework (Logframe)</h3>
          <button onClick={() => setAddingLevel(true)} className="text-xs text-catalan-primary hover:underline">+ Add Level</button>
        </div>
        {addingLevel && (
          <div className="border border-catalan-border rounded-xl p-4 mb-3 bg-catalan-primary/5 space-y-3">
            ...
            <div className="flex gap-2">
              <button disabled={!newLevel.title} onClick={addLevel} className={btnPrimary}>Add</button>
              <button onClick={() => setAddingLevel(false)} className={btnSecondary}>Cancel</button>
            </div>
          </div>
        )}
        {levels.length === 0 ? (
          <p className="text-sm text-catalan-textMuted">No logframe yet — add a Goal, then Outcomes/Outputs under it.</p>
        ) : (
          <div className="flex flex-wrap gap-2">
            {levels.map(l => (
              <span key={l.id} className="flex items-center gap-1 px-3 py-1 bg-catalan-primary/10 text-catalan-primary rounded-full text-sm">
                <span className="uppercase text-[10px] opacity-70">{l.level_type}</span> {l.title}
                {l.parent_id && <span className="text-[10px] opacity-60">← {levelLabel(l.parent_id)}</span>}
                <button onClick={() => delLevel(l.id)} className="text-catalan-primary/50 hover:text-red-500 ml-1 leading-none">×</button>
              </span>
            ))}
          </div>
        )}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-semibold text-xs text-catalan-textMuted uppercase tracking-wide">Indicators</h3>
          <button disabled={levels.length === 0} onClick={() => setAddingInd(true)} className="text-xs text-catalan-primary hover:underline disabled:opacity-40">+ Add Indicator</button>
        </div>
        {addingInd && (
          <div className="border border-catalan-border rounded-xl p-4 mb-3 bg-catalan-primary/5 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              {newInd.value_source === 'auto' && newInd.aggregation !== 'percent' && (
                <div><label className="text-xs font-medium text-catalan-textMuted">Form field</label>
                  <input list="ind-field-opts" className={`${inputSmCls} mt-0.5`} value={newInd.source_form_field}
                    onChange={e => setNewInd(p => ({ ...p, source_form_field: e.target.value }))} placeholder="field key in the form, e.g. household_income" /></div>
              )}
              {newInd.value_source === 'auto' && newInd.aggregation === 'percent' && (
                <>
                  <div><label className="text-xs font-medium text-catalan-textMuted">Numerator field</label>
                    <input list="ind-field-opts" className={`${inputSmCls} mt-0.5`} value={newInd.numerator_field}
                      onChange={e => setNewInd(p => ({ ...p, numerator_field: e.target.value }))} placeholder="e.g. nutrition_status" /></div>
                  <div><label className="text-xs font-medium text-catalan-textMuted">Numerator equals</label>
                    <input className={`${inputSmCls} mt-0.5`} value={newInd.numerator_equals}
                      onChange={e => setNewInd(p => ({ ...p, numerator_equals: e.target.value }))} placeholder="e.g. normal" /></div>
                </>
              )}
              <div><label className="text-xs font-medium text-catalan-textMuted">Disaggregate by (comma-separated field keys)</label>
                <input list="ind-field-opts" className={`${inputSmCls} mt-0.5`} value={newInd.disaggregate_by}
                  onChange={e => setNewInd(p => ({ ...p, disaggregate_by: e.target.value }))} placeholder="sex, age_band" /></div>
              <datalist id="ind-field-opts">
                {formFields.map(f => <option key={f} value={f} />)}
              </datalist>
            </div>
            <div className="flex gap-2">
              <button disabled={!newInd.name || !newInd.logframe_level_id} onClick={addIndicator} className={btnPrimary}>Add Indicator</button>
              <button onClick={() => setAddingInd(false)} className={btnSecondary}>Cancel</button>
            </div>
          </div>
        )}

        {/* Indicator Tracking Table */}
        {itt && itt.indicators.length > 0 ? (
          <div className="bg-catalan-surface border border-catalan-border rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-catalan-border flex items-center justify-between">
              <span className="text-xs text-catalan-textMuted">{itt.indicators.length} indicator(s) · {itt.waves.length} wave(s)</span>
              <button disabled={exporting} onClick={exportXlsx} className={btnSecondary}>{exporting ? '…' : '⬇ Export ITT (xlsx)'}</button>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-catalan-bg border-b border-catalan-border">
                  <tr>
                    <th className="px-3 py-2.5 text-left text-xs font-semibold text-catalan-textMuted uppercase tracking-wide">Indicator</th>
                    {itt.waves.map(w => (
                      <th key={w.id} colSpan={3} className="px-3 py-2.5 text-center text-xs font-semibold text-catalan-textMuted uppercase tracking-wide border-l border-catalan-border">
                        {w.wave_label || `Wave ${w.wave_number ?? '?'}`}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-catalan-border">
                  {itt.indicators.map(ind => (
                    <tr key={ind.id} className="hover:bg-catalan-hover transition-colors align-top">
                      <td className="px-3 py-2.5 font-medium text-catalan-text">
                        {ind.name}
                        <div className="text-[10px] text-catalan-textMuted">{ind.unit} · {ind.direction === 'increase' ? '↑ better' : '↓ better'} · {ind.value_source}
                          <button onClick={() => delIndicator(ind.id)} className="ml-2 text-red-400 hover:text-red-600">delete</button>
                        </div>
                      </td>
                      {itt.waves.map(w => {
                        const cell = ind.values_by_wave[w.id] ?? { value_id: null, target_value: null, actual_value: null, value_source: null }
                        return (
                          <td key={w.id} colSpan={3} className="px-3 py-2.5 border-l border-catalan-border">
                            <div className="flex items-center gap-2 text-xs">
                              <span className="text-catalan-textMuted">Target</span>
                              <input type="number" className="w-16 border border-catalan-border rounded px-1 py-0.5 bg-catalan-bg text-catalan-text"
                                defaultValue={cell.target_value ?? ''} disabled={!cell.value_id}
                                onBlur={e => cell.value_id && saveManual(cell.value_id, 'target_value', e.target.value)} />
                              <span className="text-catalan-textMuted">Actual</span>
                              <input type="number" className="w-16 border border-catalan-border rounded px-1 py-0.5 bg-catalan-bg text-catalan-text"
                                defaultValue={cell.actual_value ?? ''} disabled={!cell.value_id}
                                onBlur={e => cell.value_id && saveManual(cell.value_id, 'actual_value', e.target.value)} />
                            </div>
                          </td>
                        )
                      })}
                      <td className="px-3 py-2.5">
                        <button disabled={computing === ind.id} onClick={() => recompute(ind.id)} className="text-xs text-catalan-primary hover:underline">
                          {computing === ind.id ? '…' : '↻ Recompute'}
                        </button>
                      </td>
                    </tr>
                  ))}
          ...
          <p className="text-sm text-catalan-textMuted">No indicators yet.{levels.length === 0 ? ' Add a logframe level first.' : ''}</p>
        )}
      </div>
    </div>
  )
...
interface FeedbackRow {
  submission_id: string; data: Record<string, unknown>; created_at: string | null
...
const FEEDBACK_STATUS_CLS: Record<string, string> = {
  ...
  resolved: 'bg-green-100 text-green-700', escalated: 'bg-red-100 text-red-600',
}

function FeedbackPanel({ prog }: { prog: ProgramDetail }) {
  const toast = useToast()
  const [rows, setRows] = useState<FeedbackRow[]>([])
  const [staff, setStaff] = useState<StaffUser[]>([])
  const [loading, setLoading] = useState(true)
  const [statusFilter, setStatusFilter] = useState('')
  const [editing, setEditing] = useState<Record<string, { status: string; assigned_to: string; resolution_note: string }>>({})

  const load = useCallback(async () => {
    setLoading(true)
    try {
      const params = statusFilter ? { status: statusFilter } : {}
      const [fb, users] = await Promise.all([
        api.get(`/programs/${prog.id}/feedback`, { params }).then(r => r.data),
        api.get('/users/').then(r => r.data?.items ?? []).catch(() => []),
      ])
      setRows(fb); setStaff(users)
    } finally { setLoading(false) }
  }, [prog.id, statusFilter])

  useEffect(() => { load() }, [load])

  const startEdit = (row: FeedbackRow) => setEditing(p => ({
    ...p, [row.submission_id]: { status: row.status, assigned_to: row.assigned_to || '', resolution_note: row.resolution_note || '' },
  }))

  const save = async (submissionId: string) => {
    const e = editing[submissionId]; if (!e) return
    await api.patch(`/feedback/${submissionId}`, {
      status: e.status, assigned_to: e.assigned_to || null, resolution_note: e.resolution_note || null,
    })
    setEditing(p => { const n = { ...p }; delete n[submissionId]; return n })
    toast.success('Feedback updated'); load()
  }

  if (!prog.feedback_form_id) {
    return <p className="text-sm text-catalan-textMuted">No feedback form set for this program yet — pick one in <strong>Setup</strong>.</p>
  }
  if (loading) return <p className="text-center text-catalan-textMuted py-8">Loading…</p>
  ...
  return (
    <div className="space-y-4">
      ...
      {rows.length === 0 ? (
        <p className="text-sm text-catalan-textMuted">No feedback submitted yet.</p>
      ) : (
        <div className="space-y-3">
          {rows.map(row => {
            const e = editing[row.submission_id]
            const summary = Object.entries(row.data || {}).slice(0, 3).map(([k, v]) => `${k}: ${String(v)}`).join(' · ')
            return (
              <div key={row.submission_id} className="border border-catalan-border rounded-xl p-4 bg-catalan-surface">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-sm text-catalan-text">{summary || '(no summary fields)'}</div>
                    <div className="text-xs text-catalan-textMuted mt-1">
                      {row.created_at ? new Date(row.created_at).toLocaleString() : ''}
                      {row.assigned_to_name && <span className="ml-2">· Assigned: {row.assigned_to_name}</span>}
                    </div>
                  </div>
                  <Badge label={row.status.replace('_', ' ')} cls={FEEDBACK_STATUS_CLS[row.status] ?? 'bg-gray-100 text-gray-500'} />
                </div>
                {e ? (
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    <button onClick={() => setEditing(p => { const n = { ...p }; delete n[row.submission_id]; return n })} className={btnSecondary}>Cancel</button>
                  </div>
                ) : (
                  <button onClick={() => startEdit(row)} className="mt-2 text-xs text-catalan-primary hover:underline">Update status / assign</button>
                )}
              </div>
            )
          })}
...
function SetupPanel({ prog, locations, forms, onRefresh }: {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Feedback Form (MEAL accountability) */}
      <div>
        <h3 className="font-semibold text-xs text-catalan-textMuted uppercase tracking-wide mb-1">Feedback / Complaints Form</h3>
        <p className="text-xs text-catalan-textMuted mb-2">
...
export default function ProgramsPage() {
  ...
  const [detailTab, setDetailTab] = useState<'progress' | 'results' | 'feedback' | 'setup'>('progress')
            ...
            {selectedId && (
              <>
                ...
                <div className="flex gap-1 mb-4 border-b border-catalan-border">
                  {(['progress', 'results', 'feedback', 'setup'] as const).map(t => (
                    <button key={t} onClick={() => setDetailTab(t)}
                      ...
                      }`}>{t === 'setup' ? <><EmojiIcon e="⚙" /> Setup</> : t === 'results' ? <><EmojiIcon e="🎯" /> Results</> : t === 'feedback' ? <><EmojiIcon e="💬" /> Feedback</> : <><EmojiIcon e="📈" /> Progress</>}</button>
                  ))}
                </div>
                ...
                {/* Results tab */}
                {detailTab === 'results' && detail && (
                  <div className="bg-catalan-surface border border-catalan-border rounded-xl p-5">
                    <ResultsPanel prog={detail} />
                  </div>
                )}
                {detailTab === 'results' && loadingDetail && !detail && (
                  <p className="text-center text-catalan-textMuted py-8">Loading…</p>
                )}

                {/* Feedback tab */}
                {detailTab === 'feedback' && detail && (
                  <div className="bg-catalan-surface border border-catalan-border rounded-xl p-5">
                    <FeedbackPanel prog={detail} />
                  </div>
                )}
                {detailTab === 'feedback' && loadingDetail && !detail && (
                  <p className="text-center text-catalan-textMuted py-8">Loading…</p>
                )}
